...
namespace MDPro3.Utility
{
    public class TextureLoader
    {
        ...
        private static readonly SemaphoreSlim semaphoreLoadPicture = new(MAX_LOADPICTURE_THREADS);
        private static readonly SemaphoreSlim semaphoreRender = new(MAX_RENDER_THREADS);
        private static readonly SemaphoreSlim semaphoreLoadZip = new(MAX_LOADZIP_THREADS);
        ...
        public static void DeleteArt(int code)
        {
            if (cachedArts.TryGetValue(code, out var art))
                if (art.Delete())
                {
                    ...
                    UnityEngine.Object.DestroyImmediate(art.texture);
                }
        }
        ...
        public static void DeleteCard(int code)
        {
            if (cachedCards.TryGetValue(code, out var card))
            {
                if (card.Delete())
                {
                    ...
                    UnityEngine.Object.DestroyImmediate(card.texture);
                }
            }
        }
        ...
        public static void ClearCache()
        {
            ...
            foreach (var name in cachedNames.Values)
                UnityEngine.Object.Destroy(name);
            cachedArts.Clear();
            cachedCards.Clear();
            cachedNames.Clear();
        }
